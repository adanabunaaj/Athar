      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-md w-full space-y-8 text-center">
          <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-200">
            ...
            <Link
              href="/"
              className="inline-flex items-center gap-2 text-white px-6 py-3 rounded-lg font-medium transition-colors duration-200"
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-black mb-4">Join the Athar Community</h1>
          <p className="text-xl text-gray-600">Connect with Palestinian youth working towards collective success</p>
        </div>

        {error && (
          <div className="mb-6 rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-800">
            <p className="font-semibold">Couldn’t send your email.</p>
            <p className="mt-1">{error}</p>
            <p className="mt-1 text-gray-600">Tip: ensure EmailJS “Allowed origins” includes http://localhost:3000 and your Service/Template IDs are correct.</p>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8">
          ...
          <form noValidate onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <input
                id="name" name="name" required value={formData.name} onChange={handleChange}
            ...
            <div>
              <label htmlFor="age" className="block text-sm font-medium text-black mb-2">
                <Calendar className="w-4 h-4 inline mr-2" /> Age *
              </label>
              <input
                type="number" id="age" name="age" required min={17} max={60} value={formData.age} onChange={handleChange}
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors"
            ...
            <div>
              <label htmlFor="profession" className="block text-sm font-medium text-black mb-2">
                <Briefcase className="w-4 h-4 inline mr-2" /> Profession or Educational Level *
              </label>
              <input
                id="profession" name="profession" required value={formData.profession} onChange={handleChange}
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors"
                placeholder="e.g., Student at XYZ University, Software Engineer, etc."
              />
            </div>

            <div className="bg-gray-50 p-4 rounded-lg">
              <p className="text-sm text-gray-600">
                By joining, you agree to receive community updates and resources from Athar Initiative.
                You can unsubscribe at any time. We respect your privacy and will never share your information.
              </p>
            </div>

            <div className="flex gap-4">
              <Link href="/" className="flex-1 bg-gray-200 text-black px-6 py-3 rounded-lg font-medium transition-colors duration-200 hover:bg-gray-300 text-center">
                Cancel
              </Link>
              <button
                type="submit"
                disabled={isSubmitting}
                className="flex-1 text-white px-6 py-3 rounded-lg font-medium transition-colors duration-200 disabled:opacity-50"
